...
function allUniData(UniversityData) {
   let UniCards =  UniversityData.map((UniData) => {
       return `
        <div class= "UniCards">
            <h3>${UniData.name.slice(0, 22)}</h3>
            <p>${UniData.country}</p>
            <a href=${UniData.web_pages} target="_blank" class="btn">Visit Uni Website</a>
        </div>`
    });
    cardsContainer.innerHTML = UniCards.join(" ");
}
allUniData(UniversityData);


function uniSearch(elem) {
    const userInput = elem.value;
    const searchData = UniversityData.filter((data) => data.name.toLowerCase().startsWith(userInput.toLowerCase()));
    allUniData(searchData);
}